	...
	$.widget( "mobile.toolbar", {
		...
		_updateBackButton: function() {
			...
			if ( this.options.addBackBtn &&
					...
					// - there are multiple pages in the DOM
					$( ".ui-page" ).length > 1 &&
					( this.page ?

						// - if the toolbar is internal the page's URL must differ from the
						// location hash
						( this.page[ 0 ].getAttribute( "data-" + $.mobile.ns + "url" ) !==
							$.mobile.path.stripHash( location.hash ) ) :

						// - if the toolbar is external there must be at least one history item to
						// which one can go back
