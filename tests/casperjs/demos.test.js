...
casper.test.begin("demos/ should render properly", 2, function suite( test ) {
	...
	casper.then( function() {
		test.assertHttpStatus( 200 );
		test.assertExists( "li[data-filtertext='demos homepage'].ui-first-child", "First ListItem in menu panel should have class ui-first-child" );
	});
...
casper.test.begin("Checkboxradio widget should render properly", 2, function suite( test ) {
	...
	casper.then( function() {
		casper.waitUntilVisible( "ul.jqm-list > li:nth-child(5) > div > ul > li.ui-first-child > a", function() {
	...
	casper.then( function() {
		casper.waitForUrl( /\/demos\/checkboxradio-checkbox\/$/, function() {
			test.assertVisible( "input[name='checkbox-0 ']", "Checkbox checkbox-0 should be visible" );
			test.assertEvalEquals(function() {
				return $( "input[name='checkbox-0 ']" ).parent( "div" )[0].className;
			}, "ui-checkbox", "Checkbox should be enhanced" );
		});
	});
